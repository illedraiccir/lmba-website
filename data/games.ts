export type GameType = "regularSeason" | "playoffs" | "championship";
export type GameStatus = "scheduled" | "final" | "postponed" | "cancelled";
export type ResultType = "normal" | "forfeitHome" | "forfeitAway";
export type PlayoffRound = "quarterfinal" | "semifinal" | "championship";

export type Game = {
  gameId: string;
  seasonId: string;
  gameType: GameType;
  playoffRound?: PlayoffRound;
  week: number;
  date: string;
  time: string;
  location: string;
  homeTeamId: string;
  awayTeamId: string;
  firstHalfHomeScore?: number;
  firstHalfAwayScore?: number;
  secondHalfHomeScore?: number;
  secondHalfAwayScore?: number;
  homeScore?: number;
  awayScore?: number;
  overtimePeriods?: number;
  status: GameStatus;
  resultType?: ResultType;
  headline?: string;
  recapTeaser?: string;
};

export const games: Game[] = [
  {
    "gameId": "2026-w1-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 1,
    "date": "2026-03-31",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "ball-starz",
    "awayTeamId": "lmba-jam",
    "firstHalfHomeScore": 33,
    "firstHalfAwayScore": 19,
    "secondHalfHomeScore": 28,
    "secondHalfAwayScore": 30,
    "homeScore": 61,
    "awayScore": 49,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Get Revenge In Championship Rematch Behind Guy Johnson’s 23",
    "recapTeaser": "Opening Night delivered an instant playoff-caliber showdown as Ball Starz knocked off defending champion LMBA Jam in a rematch of last season’s title game. Reigning MVP Guy Johnson led the charge with 23 points to help Ball Starz start 2026 with an early statement win."
  },
  {
    "gameId": "2026-w1-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 1,
    "date": "2026-03-31",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "department-of-offense",
    "awayTeamId": "thrillers",
    "firstHalfHomeScore": 34,
    "firstHalfAwayScore": 24,
    "secondHalfHomeScore": 20,
    "secondHalfAwayScore": 32,
    "homeScore": 54,
    "awayScore": 56,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Thrillers Storm Back From 10 Down Behind Adams And Ziller",
    "recapTeaser": "Thrillers erased a double-digit halftime deficit to steal a gritty 56-53 Opening Night win over Department of Offense. Doug Adams poured in 23 points while Brett Ziller added 12 as Thrillers closed the game with a dominant second-half rally."
  },
  {
    "gameId": "2026-w2-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-13",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "prestige-worldwide",
    "awayTeamId": "hash-slinging-slashers",
    "firstHalfHomeScore": 18,
    "firstHalfAwayScore": 30,
    "secondHalfHomeScore": 15,
    "secondHalfAwayScore": 18,
    "homeScore": 33,
    "awayScore": 48,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Hash-Slinging Slashers Clamp Down Early To Cruise Past Prestige Worldwide",
    "recapTeaser": "Hash-Slinging Slashers controlled the game from the opening tip, building a 12-point halftime lead before finishing off a comfortable 48-33 win over Prestige Worldwide. Strong defense and balanced scoring helped the Slashers improve to 1-0 on the young season."
  },
  {
    "gameId": "2026-w2-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-13",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "free-agent-team",
    "awayTeamId": "thrillers",
    "homeScore": 41,
    "awayScore": 38,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Nkenna drains long-range, step-back 3 at the buzzer to lift Free Agents over Thrillers",
    "recapTeaser": "Free-Agent Team pulled off a thrilling 41-38 victory over Thrillers thanks to a clutch buzzer-beater from sharpshooter Nkenna. With the game tied and the clock winding down, Nkenna stepped back beyond the arc and drained a deep three-pointer as time expired to secure the win for Free Agents in a game that had been neck-and-neck throughout. The victory marks a strong start for Free-Agent Team, while Thrillers will look to bounce back in their next outing."
  },
  {
    "gameId": "2026-w2-g03",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-13",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "department-of-offense",
    "awayTeamId": "ball-starz",
    "firstHalfHomeScore": 19,
    "firstHalfAwayScore": 24,
    "secondHalfHomeScore": 25,
    "secondHalfAwayScore": 30,
    "homeScore": 44,
    "awayScore": 54,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Stay Unbeaten Behind Guy Johnson’s Five Threes",
    "recapTeaser": "Ball Starz used a balanced offensive attack and another explosive outing from Guy Johnson to defeat Department of Offense 54-44. Johnson knocked down five triples and scored 19 points as Ball Starz controlled the game from start to finish."
  },
  {
    "gameId": "2026-w2-g04",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-14",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "trust-the-process",
    "awayTeamId": "prestige-worldwide",
    "homeScore": 52,
    "awayScore": 40,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Trust The Process Pull Away Late To Secure First Win Of Season",
    "recapTeaser": "Trust The Process turned a competitive matchup into a comfortable double-digit victory in the second half, defeating Prestige Worldwide 52-40. A strong defensive effort helped TTP lock down the game late and even their record."
  },
  {
    "gameId": "2026-w2-g05",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-14",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "lmba-jam",
    "awayTeamId": "free-agent-team",
    "homeScore": 70,
    "awayScore": 39,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "LMBA Jam Explodes For 70 In Statement Win Over Free Agent Team",
    "recapTeaser": "LMBA Jam delivered one of the league’s most dominant offensive performances of the season, rolling past the Free Agent Team 70-39. LMBA Jam’s relentless pace and transition offense overwhelmed the Free Agents from the opening minutes."
  },
  {
    "gameId": "2026-w2-g06",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-16",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "hash-slinging-slashers",
    "awayTeamId": "lmba-jam",
    "firstHalfHomeScore": 20,
    "firstHalfAwayScore": 20,
    "secondHalfHomeScore": 29,
    "secondHalfAwayScore": 12,
    "homeScore": 49,
    "awayScore": 32,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Hash-Slinging Slashers Dominate Second Half To Upset Short-Handed LMBA Jam",
    "recapTeaser": "After entering halftime tied at 20, Hash-Slinging Slashers completely took over in the second half to defeat a severely depleted LMBA Jam squad 49-32. LMBA Jam was limited to just five available players and missing key contributors Shelepets, Homsher, Brusseler, and Ziller, as the Slashers outscored them 29-12 after the break in one of the league’s strongest defensive performances of the season."
  },
  {
    "gameId": "2026-w2-g07",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-16",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "ball-starz",
    "awayTeamId": "thrillers",
    "firstHalfHomeScore": 26,
    "firstHalfAwayScore": 19,
    "secondHalfHomeScore": 19,
    "secondHalfAwayScore": 34,
    "homeScore": 45,
    "awayScore": 53,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Thrillers Rally Again, Erase Seven-Point Halftime Deficit Against Ball Starz",
    "recapTeaser": "For the second time in two weeks, Thrillers stormed back in the second half to steal a win, defeating Ball Starz 53-45. After trailing by seven at halftime, Thrillers exploded for 34 second-half points behind Doug Adams, who poured in 20 points overall with 12 coming after the break to fuel another comeback victory."
  },
  {
    "gameId": "2026-w2-g08",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 2,
    "date": "2026-04-16",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "trust-the-process",
    "awayTeamId": "department-of-offense",
    "firstHalfHomeScore": 20,
    "firstHalfAwayScore": 34,
    "secondHalfHomeScore": 36,
    "secondHalfAwayScore": 35,
    "homeScore": 56,
    "awayScore": 69,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Department Of Offense Erupts For 69 In High-Scoring Win Over Trust The Process",
    "recapTeaser": "Department of Offense jumped out to a 14-point halftime lead and never looked back in a 69-56 victory over Trust The Process. Even after TTP mounted a stronger second half, D.O.O. continued to answer offensively to secure the win."
  },
  {
    "gameId": "2026-w3-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-20",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "hash-slinging-slashers",
    "awayTeamId": "free-agent-team",
    "firstHalfHomeScore": 29,
    "firstHalfAwayScore": 10,
    "secondHalfHomeScore": 32,
    "secondHalfAwayScore": 22,
    "homeScore": 61,
    "awayScore": 32,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Hash-Slinging Slashers Continue Defensive Tear In Blowout Win",
    "recapTeaser": "Hash-Slinging Slashers suffocated the Free Agent Team defensively from start to finish, cruising to a 61-32 victory. The Slashers built a 19-point halftime lead and never allowed the Free Agents back into the game."
  },
  {
    "gameId": "2026-w3-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-20",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "prestige-worldwide",
    "awayTeamId": "trust-the-process",
    "firstHalfHomeScore": 19,
    "firstHalfAwayScore": 23,
    "secondHalfHomeScore": 23,
    "secondHalfAwayScore": 37,
    "homeScore": 42,
    "awayScore": 60,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Trust The Process Pull Away Late To Stay Hot",
    "recapTeaser": "Trust The Process used a huge second half to break open a close game and defeat Prestige Worldwide 60-42. After leading by just four at halftime, TTP outscored Prestige 37-23 after the break behind a dominant second-half performance from Hakim Jackson, who scored 13 of his 17 points after halftime."
  },
  {
    "gameId": "2026-w3-g03",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-22",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "lmba-jam",
    "awayTeamId": "department-of-offense",
    "homeScore": 65,
    "awayScore": 55,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "LMBA Jam Outlast Department Of Offense In Offensive Showcase",
    "recapTeaser": "LMBA Jam continued its offensive resurgence with a 65-55 win over Department of Offense. LMBA Jam controlled the pace throughout the night and held off multiple D.O.O. runs to close out the 10-point victory."
  },
  {
    "gameId": "2026-w3-g04",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-22",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "free-agent-team",
    "awayTeamId": "ball-starz",
    "homeScore": 41,
    "awayScore": 90,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Turn Wednesday Night Into A Track Meet In 90-41 Rout",
    "recapTeaser": "Ball Starz delivered the most explosive offensive performance of the season, overwhelming the Free Agent Team 90-41. The Ball Starz pushed the tempo from the opening tip and never let up, turning the matchup into a full-court track meet while doubling up Free Agent Team on the scoreboard."
  },
  {
    "gameId": "2026-w3-g05",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-22",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "trust-the-process",
    "awayTeamId": "thrillers",
    "homeScore": 64,
    "awayScore": 48,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Trust The Process Lock Down Thrillers In Statement Victory",
    "recapTeaser": "Trust The Process continued its strong midseason run with a convincing 64-48 win over Thrillers. TTP controlled both ends of the floor throughout the game and never allowed Thrillers to build consistent momentum."
  },
  {
    "gameId": "2026-w3-g06",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-23",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "thrillers",
    "awayTeamId": "lmba-jam",
    "firstHalfHomeScore": 15,
    "firstHalfAwayScore": 34,
    "secondHalfHomeScore": 13,
    "secondHalfAwayScore": 32,
    "homeScore": 28,
    "awayScore": 66,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "LMBA Jam Steamroll Thrillers Behind Massive First Half",
    "recapTeaser": "LMBA Jam buried Thrillers early and never looked back in a dominant 66-28 victory. LMBA Jam raced out to a 34-15 halftime lead before completely shutting the door with another huge second half behind Anthony Shelepets, who poured in 26 points in the win."
  },
  {
    "gameId": "2026-w3-g07",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 3,
    "date": "2026-04-23",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "hash-slinging-slashers",
    "awayTeamId": "prestige-worldwide",
    "firstHalfHomeScore": 29,
    "firstHalfAwayScore": 20,
    "secondHalfHomeScore": 25,
    "secondHalfAwayScore": 25,
    "homeScore": 54,
    "awayScore": 45,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Hash-Slinging Slashers Stay Red Hot With Wire-To-Wire Win",
    "recapTeaser": "Hash-Slinging Slashers continued their impressive run with a 54-45 victory over Prestige Worldwide. The Slashers led by nine at halftime and matched Prestige basket-for-basket in the second half to finish off the win."
  },
  {
    "gameId": "2026-w4-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 4,
    "date": "2026-04-28",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "trust-the-process",
    "awayTeamId": "free-agent-team",
    "homeScore": 83,
    "awayScore": 55,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Trust The Process Explodes For 83 In Blowout Victory",
    "recapTeaser": "Trust The Process explode offensively in an 83-55 rout of the Free Agent Team. TTP pushed the pace from the opening tip and never slowed down."
  },
  {
    "gameId": "2026-w4-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 4,
    "date": "2026-04-28",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "department-of-offense",
    "awayTeamId": "prestige-worldwide",
    "homeScore": 45,
    "awayScore": 48,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Prestige Worldwide Sneak Past Department Of Offense In Defensive Battle For First Win",
    "recapTeaser": "Prestige Worldwide survived a low-scoring thriller against Department of Offense, escaping with a 48-45 win for their first victory of the season. In his season debut, Travis Gibbs sparked Prestige with 15 points and five three-pointers, while timely defensive stops and clutch late possessions sealed the narrow win."
  },
  {
    "gameId": "2026-w4-g03",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 4,
    "date": "2026-04-28",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "ball-starz",
    "awayTeamId": "hash-slinging-slashers",
    "firstHalfHomeScore": 26,
    "firstHalfAwayScore": 15,
    "secondHalfHomeScore": 24,
    "secondHalfAwayScore": 10,
    "homeScore": 50,
    "awayScore": 25,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Defense Suffocates Slashers In 25-Point Blowout",
    "recapTeaser": "Ball Starz delivered a dominant two-way performance in a 50-25 dismantling of Hash-Slinging Slashers. After building an 11-point halftime advantage, the Starz completely shut down the Slashers offense in the second half."
  },
  {
    "gameId": "2026-w4-g04",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 4,
    "date": "2026-04-29",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "lmba-jam",
    "awayTeamId": "prestige-worldwide",
    "firstHalfHomeScore": 32,
    "firstHalfAwayScore": 32,
    "secondHalfHomeScore": 27,
    "secondHalfAwayScore": 16,
    "homeScore": 59,
    "awayScore": 48,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "LMBA Jam Pull Away Late After First-Half Shootout",
    "recapTeaser": "LMBA Jam broke open a tied game after halftime to defeat Prestige Worldwide 59-48. After both teams entered the break deadlocked at 32, LMBA Jam tightened up defensively and controlled the final stretch."
  },
  {
    "gameId": "2026-w4-g05",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 4,
    "date": "2026-04-29",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "hash-slinging-slashers",
    "awayTeamId": "trust-the-process",
    "firstHalfHomeScore": 31,
    "firstHalfAwayScore": 26,
    "secondHalfHomeScore": 22,
    "secondHalfAwayScore": 27,
    "homeScore": 53,
    "awayScore": 62,
    "overtimePeriods": 1,
    "status": "final",
    "resultType": "normal",
    "headline": "Trust The Process Survive Overtime Thriller Against Slashers",
    "recapTeaser": "Trust The Process erased a five-point halftime deficit and survived overtime to edge Hash-Slinging Slashers 62-53. TTP closed regulation strong before taking complete control in the extra period."
  },
  {
    "gameId": "2026-w5-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 5,
    "date": "2026-05-04",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "free-agent-team",
    "awayTeamId": "ball-starz",
    "firstHalfHomeScore": 19,
    "firstHalfAwayScore": 41,
    "secondHalfHomeScore": 17,
    "secondHalfAwayScore": 49,
    "homeScore": 36,
    "awayScore": 90,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Drop Another 90-Piece In Rout Of Free Agent Team",
    "recapTeaser": "Ball Starz continued their offensive dominance with a second 90-point performance in the previous three games, crushing the Free Agent Team 90-36. The Ball Starz led by 22 at halftime before exploding for 49 second-half points."
  },
  {
    "gameId": "2026-w5-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 5,
    "date": "2026-05-04",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "thrillers",
    "awayTeamId": "department-of-offense",
    "firstHalfHomeScore": 23,
    "firstHalfAwayScore": 42,
    "secondHalfHomeScore": 29,
    "secondHalfAwayScore": 19,
    "homeScore": 52,
    "awayScore": 61,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Department Of Offense Hold Off Furious Thrillers Comeback",
    "recapTeaser": "Department of Offense looked headed for a blowout after building a 19-point halftime lead, but Thrillers mounted a huge second-half rally before ultimately falling 61-52. D.O.O. did just enough late to escape with the win."
  },
  {
    "gameId": "2026-w5-g03",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 5,
    "date": "2026-05-05",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "ball-starz",
    "awayTeamId": "thrillers",
    "firstHalfHomeScore": 34,
    "firstHalfAwayScore": 17,
    "secondHalfHomeScore": 24,
    "secondHalfAwayScore": 25,
    "homeScore": 58,
    "awayScore": 42,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Ball Starz Cruise Past Thrillers Behind Dominant First Half",
    "recapTeaser": "Ball Starz got their revenge from their early season loss to the Thrillers. Ball Starzjumped out early and never allowed Thrillers back into the game in a 58-42 victory. The Ball Starz carried a 17-point lead into halftime and controlled the tempo throughout the night."
  },
  {
    "gameId": "2026-w5-g04",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 5,
    "date": "2026-05-05",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "hash-slinging-slashers",
    "awayTeamId": "department-of-offense",
    "homeScore": 20,
    "awayScore": 60,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "forfeitHome",
    "headline": "Department Of Offense Awarded Forfeit Victory Over Slashers",
    "recapTeaser": "Department of Offense earned a 60-20 forfeit victory after Hash-Slinging Slashers were unable to field a valid lineup. The result keeps D.O.O. firmly in the playoff seeding mix heading into Week 6."
  },
  {
    "gameId": "2026-w5-g05",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 5,
    "date": "2026-05-05",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "lmba-jam",
    "awayTeamId": "trust-the-process",
    "firstHalfHomeScore": 31,
    "firstHalfAwayScore": 29,
    "secondHalfHomeScore": 41,
    "secondHalfAwayScore": 31,
    "homeScore": 72,
    "awayScore": 60,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "LMBA Jam Win Offensive Shootout Against Trust The Process",
    "recapTeaser": "LMBA Jam outlasted Trust The Process in one of the season’s highest-scoring games, earning a 72-60 victory. LMBA Jam led by just two at halftime before pulling away behind a massive 41-point second half."
  },
  {
    "gameId": "2026-w6-g01",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-11",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "free-agent-team",
    "awayTeamId": "department-of-offense",
    "firstHalfHomeScore": 15,
    "firstHalfAwayScore": 36,
    "secondHalfHomeScore": 5,
    "secondHalfAwayScore": 35,
    "homeScore": 20,
    "awayScore": 71,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Department Of Offense Deliver Week 6 Beatdown Of Free Agent Team",
    "recapTeaser": "Department of Offense overwhelmed the Free Agent Team from the opening tip in a lopsided 71-20 victory. D.O.O. built a 21-point halftime lead and completely shut the door by allowing just five second-half points."
  },
  {
    "gameId": "2026-w6-g02",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-11",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "prestige-worldwide",
    "awayTeamId": "ball-starz",
    "firstHalfHomeScore": 18,
    "firstHalfAwayScore": 46,
    "secondHalfHomeScore": 30,
    "secondHalfAwayScore": 30,
    "homeScore": 48,
    "awayScore": 76,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "normal",
    "headline": "Thin Ball Starz Squad Handle Week 6 Challenge From Prestige Worldwide With Ease",
    "recapTeaser": "Ball Starz continued their dominant season with a convincing 76-48 win over Prestige Worldwide. MVP Front-Runner Guy Johnson was a surprise no-show come tip-off. Despite no bench, the Ball Starz built a 28-point halftime lead and cruised to another victory."
  },
  {
    "gameId": "2026-w6-g03",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-12",
    "time": "6:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "lmba-jam",
    "awayTeamId": "hash-slinging-slashers",
    "status": "final",
    "firstHalfHomeScore": 27,
    "firstHalfAwayScore": 30,
    "secondHalfHomeScore": 40,
    "secondHalfAwayScore": 34,
    "homeScore": 67,
    "awayScore": 64,
    "overtimePeriods": 0,
    "resultType": "normal",
    "headline": "LMBA Jam Explode For 40 Second-Half Points To Outlast Hash-Slinging Slashers",
    "recapTeaser": "LMBA Jam exploded for 40 second-half points to erase a halftime deficit and defeat Hash-Slinging Slashers 67-64 in a Week 6 thriller. Chris Clemente buried four second-half threes while Jam sealed the win at the line, finishing a perfect 7-for-7 on clutch free throws down the stretch."
  },
  {
    "gameId": "2026-w6-g04",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-12",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "thrillers",
    "awayTeamId": "trust-the-process",
    "status": "final",
    "homeScore": 50,
    "awayScore": 82,
    "overtimePeriods": 0,
    "resultType": "normal",
    "headline": "Trust The Process Explode For 82 In Offensive Clinic Against Thrillers",
    "recapTeaser": "Trust The Process turned in one of the league’s most complete offensive performances of the season, rolling past Thrillers 82-50 in Week 6. TTP buried 13 three-pointers and featured a balanced scoring attack led by Kevin Tierney’s 18 points and Jordan Wilcox’s 17, while Doug Adams kept Thrillers competitive early with a game-high 27 points and five triples."
  },
  {
    "gameId": "2026-w6-g05",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-12",
    "time": "8:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "prestige-worldwide",
    "awayTeamId": "free-agent-team",
    "homeScore": 60,
    "awayScore": 20,
    "overtimePeriods": 0,
    "status": "final",
    "resultType": "forfeitAway",
    "headline": "Prestige Worldwide Awarded Forfeit Victory Over Free Agents",
    "recapTeaser": "Prestige Worldwide earned a 60-20 forfeit victory after Free Agents were unable to field a valid lineup. The result gives Prestige Worldwide their second win of the season to vie for better playoff seeding ahead of Week 7."
  },
  {
    "gameId": "2026-w6-g06",
    "seasonId": "2026",
    "gameType": "regularSeason",
    "week": 6,
    "date": "2026-05-14",
    "time": "7:30 PM",
    "location": "Moorestown Upper Elementary",
    "homeTeamId": "ball-starz",
    "awayTeamId": "trust-the-process",
    "status": "final",
    "firstHalfHomeScore": 29,
    "firstHalfAwayScore": 41,
    "secondHalfHomeScore": 41,
    "secondHalfAwayScore": 27,
    "homeScore": 70,
    "awayScore": 68,
    "overtimePeriods": 0,
    "resultType": "normal",
    "headline": "INSTANT CLASSIC! Ball Starz Close On 35-10 Run To Erase 23-Point Second-Half Deficit",
    "recapTeaser": "In one of the wildest games in recent history, Ball Starz stormed back from a 35-58 second-half deficit with under 12 minutes remaining to stun Trust The Process 70-68. Contributions from a multitude of players were critical down the final 12-minute stretch; Ricciardelli (8pts), Fontanazza (8pts), Denson (10pts), and Guy (11 pts) . Guy Johnson drilled a game-tying three with 13 seconds left before a costly TTP backcourt violation gave Ball Starz one final chance. Moments later, Ryan Fontanazza was fouled on a three-point attempt with 3.5 seconds remaining and calmly knocked down two clutch free throws to complete the comeback. Trust The Process repeatedly missed critical one-and-one FT opportunities down the stretch, opening the door for the furious 35-10 Ball Starz finish, while Jeff Savage’s half-court buzzer heave nearly stole the game back at the horn."
  },
  {
    gameId: "2026-w6-g07",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 6,
    date: "2026-05-14",
    time: "8:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "hash-slinging-slashers",
    homeTeamId: "thrillers",
    firstHalfAwayScore: 19,
    firstHalfHomeScore: 24,
    secondHalfAwayScore: 24,
    secondHalfHomeScore: 25,
    awayScore: 43,
    homeScore: 49,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Thrillers Hold Off Hash-Slinging Slashers Behind Ziller’s 24-Point Night",
    recapTeaser: "Thrillers controlled the game just enough to survive Hash-Slinging Slashers, earning a 49-43 Week 6 win. Brett Ziller led the way with 24 points and three triples, while Doug Adams added 13 as Thrillers held off a balanced Slashers effort.",
  },
  {
    gameId: "2026-w7-g01",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-18",
    time: "6:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "prestige-worldwide",
    homeTeamId: "thrillers",
    firstHalfAwayScore: 46,
    firstHalfHomeScore: 29,
    secondHalfAwayScore: 14,
    secondHalfHomeScore: 16,
    awayScore: 60,
    homeScore: 45,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Prestige Worldwide Cruise Past Thrillers Behind Balanced Attack",
    recapTeaser:
      "Prestige Worldwide closed out the regular season with a convincing 60-45 win over Thrillers. Thomas Hays led the way with 19 points while Kevin Carroll knocked down four triples as Prestige controlled the game early and never looked back.",
  },
  {
    gameId: "2026-w7-g02",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-18",
    time: "7:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "trust-the-process",
    homeTeamId: "free-agent-team",
    firstHalfAwayScore: undefined,
    firstHalfHomeScore: undefined,
    secondHalfAwayScore: undefined,
    secondHalfHomeScore: undefined,
    awayScore: 60,
    homeScore: 20,
    overtimePeriods: 0,
    status: "final",
    resultType: "forfeitAway",
    headline: "Trust The Process Awarded Week 7 Forfeit Victory Over Free Agent Team",
    recapTeaser:
      "Trust The Process closed out the regular season with a 60-20 forfeit victory over Free Agent Team. The win keeps TTP near the top of the standings heading into the postseason.",
  },
  {
    gameId: "2026-w7-g03",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-18",
    time: "8:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "lmba-jam",
    homeTeamId: "department-of-offense",
    firstHalfAwayScore: 31,
    firstHalfHomeScore: 26,
    secondHalfAwayScore: 33,
    secondHalfHomeScore: 34,
    awayScore: 64,
    homeScore: 60,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "LMBA Jam Survive Department Of Offense To Keep #1 Seed Hopes Alive",
    recapTeaser:
      "LMBA Jam withstood a furious second-half charge from Department of Offense and escaped with a 64-60 victory. Mike Ziller scored 19 points while Anthony Shelepets added 16 as LMBA Jam secured a crucial win to set up their season finale against the Ball Starz for the #1 seed.",
  },
  {
    gameId: "2026-w7-g04",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-19",
    time: "6:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "department-of-offense",
    homeTeamId: "prestige-worldwide",
    firstHalfAwayScore: 27,
    firstHalfHomeScore: 24,
    secondHalfAwayScore: 33,
    secondHalfHomeScore: 25,
    awayScore: 60,
    homeScore: 49,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Department Of Offense Pull Away Late To Finish Regular Season Strong",
    recapTeaser:
      "Department of Offense outscored Prestige Worldwide by eight in the second half to secure a 60-49 victory. Kyle Kelly scored 19 points, Grant Clark added 17, and Deven Cobb chipped in 13 as DOO built momentum heading into the playoffs.",
  },
  {
    gameId: "2026-w7-g05",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-19",
    time: "7:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "ball-starz",
    homeTeamId: "lmba-jam",
    firstHalfAwayScore: 31,
    firstHalfHomeScore: 26,
    secondHalfAwayScore: 37,
    secondHalfHomeScore: 9,
    awayScore: 68,
    homeScore: 35,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Ball Starz Dominate LMBA Jam To Clinch No. 1 Seed",
    recapTeaser:
      "With the No. 1 seed on the line, Ball Starz delivered their biggest statement of the season. After leading by just five at halftime, Ball Starz exploded for a 37-9 second half to rout LMBA Jam 68-35 and secure the top seed entering the playoffs.",
  },
  {
    gameId: "2026-w7-g06",
    seasonId: "2026",
    gameType: "regularSeason",
    week: 7,
    date: "2026-05-19",
    time: "8:30 PM",
    location: "Moorestown Upper Elementary",
    awayTeamId: "free-agent-team",
    homeTeamId: "hash-slinging-slashers",
    firstHalfAwayScore: 12,
    firstHalfHomeScore: 35,
    secondHalfAwayScore: 12,
    secondHalfHomeScore: 23,
    awayScore: 24,
    homeScore: 58,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Hash-Slinging Slashers Close Regular Season With Wire-To-Wire Win To End 5-Game Losing Streak",
    recapTeaser:
      "Hash-Slinging Slashers jumped out to a 35-12 halftime lead and never looked back, defeating the Free Agent Team 58-24 in the regular-season finale. Connor Thomas scored 14 points, Courtney Cubbage added 13, and Keshon Coleman chipped in 11 as the Slashers finished the regular season at 5-5.",
  },
  {
    gameId: "2026-p-qf-g01",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "quarterfinal",
    week: 8,
    date: "2026-06-02",
    time: "7:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "ball-starz",
    awayTeamId: "free-agent-team",
    homeScore: 60,
    awayScore: 20,
    overtimePeriods: 0,
    status: "final",
    resultType: "forfeitAway",
    headline: "Ball Starz Advance After Free Agent Team Forfeits Quarterfinal",
    recapTeaser:
      "Ball Starz advanced to the semifinals without having to take the floor, as Free Agent Team forfeited their quarterfinal matchup. After dropping 90 points on Free Agent Team in both regular-season meetings, Ball Starz move on while the No. 8 seed exits without making an appearance.",
  },
  {
    gameId: "2026-p-qf-g02",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "quarterfinal",
    week: 8,
    date: "2026-06-02",
    time: "8:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "lmba-jam",
    awayTeamId: "thrillers",
    firstHalfHomeScore: 31,
    firstHalfAwayScore: 24,
    secondHalfHomeScore: 16,
    secondHalfAwayScore: 23,
    homeScore: 51,
    awayScore: 52,
    overtimePeriods: 1,
    status: "final",
    resultType: "normal",
    headline: "Tailor’s Corner Three At The Buzzer Stuns Defending Champs In Overtime Classic",
    recapTeaser:
      "In the game of the year thus far, Thrillers erased multiple double-digit second-half deficits and stunned defending champion LMBA Jam 52-51 in overtime. Dave Homsher looked like he had saved LMBA Jam with a deep, contested three in the final minute of OT, but Manthan Tailor answered with his first shot of the game — a wide-open corner three at the buzzer — to send Thrillers to the semifinals and end LMBA Jam’s title defense.",
  },
  {
    gameId: "2026-p-qf-g03",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "quarterfinal",
    week: 8,
    date: "2026-06-03",
    time: "7:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "trust-the-process",
    awayTeamId: "prestige-worldwide",
    firstHalfHomeScore: 46,
    firstHalfAwayScore: 31,
    secondHalfHomeScore: 21,
    secondHalfAwayScore: 15,
    homeScore: 67,
    awayScore: 46,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Trust The Process Shuts Down Prestige Worldwide To Reach Semifinals",
    recapTeaser:
      "Trust The Process advanced to the semifinals with a convincing 67-46 victory over Prestige Worldwide. TTP built a double-digit halftime lead behind balanced scoring from Zach Rollman, Kevin Tierney, Douglas Alexander, and Jordan Wilcox, then slammed the door with a suffocating defensive effort that held Prestige star Thomas Hays scoreless.",
  },
  {
    gameId: "2026-p-qf-g04",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "quarterfinal",
    week: 8,
    date: "2026-06-03",
    time: "6:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "department-of-offense",
    awayTeamId: "hash-slinging-slashers",
    firstHalfHomeScore: 24,
    firstHalfAwayScore: 30,
    secondHalfHomeScore: 26,
    secondHalfAwayScore: 19,
    homeScore: 50,
    awayScore: 49,
    overtimePeriods: 0,
    status: "final",
    resultType: "normal",
    headline: "Five-Man Department Of Offense Rallies Past Hash-Slinging Slashers",
    recapTeaser:
      "Playing with only five available players, Department of Offense erased a six-point halftime deficit and survived a tense finish to defeat Hash-Slinging Slashers 50-49. Eli Ratliff scored 17 points while the short-handed squad outscored HSH 26-19 in the second half to punch its ticket to the semifinals.",
  },
  {
    gameId: "2026-p-sf-g01",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "semifinal",
    week: 9,
    date: "2026-06-08",
    time: "7:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "ball-starz",
    awayTeamId: "department-of-offense",
    status: "scheduled",
    resultType: "normal",
    headline: "Top-Seeded Ball Starz Begin Title Quest Against Battle-Tested DOO",
    recapTeaser:
      "Ball Starz finally take the floor in the postseason after advancing through a quarterfinal forfeit. Standing in their way is a Department of Offense squad that overcame a halftime deficit with only five players to earn a semifinal showdown.",
  },

  {
    gameId: "2026-p-sf-g02",
    seasonId: "2026",
    gameType: "playoffs",
    playoffRound: "semifinal",
    week: 9,
    date: "2026-06-08",
    time: "8:30 PM",
    location: "Moorestown Upper Elementary",
    homeTeamId: "trust-the-process",
    awayTeamId: "thrillers",
    status: "scheduled",
    resultType: "normal",
    headline: "Cinderella Thrillers Face Defensive Juggernaut Trust The Process",
    recapTeaser:
      "Fresh off a stunning overtime upset of defending champion LMBA Jam, the seventh-seeded Thrillers look to continue their playoff run against a Trust The Process team that dominated Prestige Worldwide to reach the semifinals.",
  },
];
